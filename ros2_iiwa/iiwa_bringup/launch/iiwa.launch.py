...
def generate_launch_description():
    ...
    declared_arguments.append(
        DeclareLaunchArgument(
            'blob_detection',
            default_value='false',
            description='Select if use blob detection or not',
        )
    )
    ...
    blob_detection = LaunchConfiguration('blob_detection')
    ...
    bridge_camera = Node(
        ...
        arguments=[
            ...
            '--ros-args', 
            '-r', '/camera:=/videocamera', '-r', '/camera_info:=/cam_info',
        ],
        output='screen',
        condition=IfCondition(use_vision),
    )

    #delayed_controller_manager = TimerAction(period=50, actions=[control_node])
